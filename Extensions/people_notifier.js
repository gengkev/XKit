		...
		GM_xmlhttpRequest({
			...
			onload: function(response) {
				...
				try {
					...
					console.log("people-notifier -> doing blog " + url);
					...
					while (do_continue_lads) {
						...
						if (typeof obj.last_post_id !== "undefined") {
						...
						/*if (obj.last_20_posts.indexOf(data.posts[lad_count].id) !== -1) { 
							console.log("people-notifier -> Skipping, on last_20_posts list.");
							continue; 
						}*/
					...
					if (found_count > 0) {
						...
						XKit.extensions.people_notifier.set_read_count(url, found_count);
						
						$("#xkit-people-notifier-for---" + url).find(".count").html(found_count);	
	...
	list_blogs: function() {
		...
		$(".xkit-people-notifier-person").bind("click", function(event) {
			...
			if ($(this).hasClass("people-notifier-error")) {
				XKit.window.show("Error fetching.", "<b>Blog Tracker was unable to fetch information about the blog \"" + $(this).attr('data-url') + "\".</b><div style=\"margin-top: 15px;\">It might be due to a recent Tumblr change or bug, or that the person changed their URL. Try refreshing the page or click on Open In New Tab to check if they have deleted their blog or changed their URL.</div>","error","<a href=\"#\" target=\"_BLANK\" class=\"xkit-button default\">Open in new tab</a><div class=\"xkit-button\" id=\"people-notifier-delete-this\">Remove from track list</div><div class=\"xkit-button\" id=\"xkit-close-message\">Cancel</div>");
